...
Define a function to get a Palindrome:
    ...
    nested in this function define a function toChars to make all characters lowercase with only letters
        set the word to lowercase
            ...
            for all letters in the string:
                ...
                print the letterstring as it is passed through the function
            return letterstring

    nested in the Palidrome function is isPal:
        print statement showing the function is called
        if the length of the word is less than or equal to 1
            Print that the base case is about to be returned
    ...
    print that the result is about to be returned
    ...
    def toChars (s):
        ...
        for c in s:
            if c in 'abcdefghijklmnopqrstuvwxyz':
                letterstring += c
            print ('This is the string in the toChars function: ', letterstring)
        return letterstring

    def isPal (s):
        '''Checks if word is a palindrome'''
        print ('isPal function called with arguenment', s)
        if len (s) <= 1:
            print ('About to return True from isPal from the base case')
            return True
        
        else:
            palcheck = s[0] == s [-1] and isPal (s[1:-1])    
            print ('This is in the isPal function:', palcheck)
            return palcheck
